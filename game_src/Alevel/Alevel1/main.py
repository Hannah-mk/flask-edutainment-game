...
async def main():
    ...
    # Optionally let the loader know we’re up
    ...
    while running:
        ...
        if current_state == STATE_OVERVIEW:
            ...
            # Use SRCALPHA for proper transparency in WASM builds
            ...
            for switch in switches:
                img = switch["alt_image"] if switch["state"] else switch["image"]
                screen.blit(img, switch["rect"])
            ...
            screen.blit(font.render("Use two resistors in parallel + one in series!", True, WHITE), (50, 63))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                pos = event.pos
                if current_state == STATE_OVERVIEW and initialclick.collidepoint(pos):
                    current_state = STATE_ZOOM
                elif current_state == STATE_ZOOM:
                    for name, comp in components.items():
                        if comp["rect"].collidepoint(pos) and not comp["placed"] and name not in ("battery", "led"):
                            selected_component = name
                            mouse_offset = (pos[0] - comp["rect"].x, pos[1] - comp["rect"].y)
                            break
                    for switch in switches:
                        if switch["rect"].collidepoint(pos):
        ...
        correct = (
            "resistor_1k"   in placed and parallel_area.colliderect(components["resistor_1k"]["rect"]) and
            "resistor_1_5k" in placed and parallel_area.colliderect(components["resistor_1_5k"]["rect"]) and
            "resistor_100"  in placed and series_area.colliderect(components["resistor_100"]["rect"]) and
            all(s["state"] for s in switches)
        )
        if correct:
            components["led"]["image"] = led_on_image
            # tell the parent frame we’ve completed
            window.parent.postMessage("level_complete_gcse1", "*")

        pygame.display.flip()

        # throttle at ~60 FPS so the browser can breathe
